# Shared Python module (DB access, HTTP helpers, error mapping, auth) is the
# single source of truth in backend/_shared/shared and is vendored into each
# service's own folder by bin/sync-shared.sh before packaging, so it ships
# inside each function's deployment package. (A Lambda layer would be cleaner,
# but the workshop participant role is not granted lambda:PublishLayerVersion.)
module "lambda" {
